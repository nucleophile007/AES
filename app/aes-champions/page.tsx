...
// Fallback testimonials for AES Champions
const fallbackTestimonials = [
  {
    id: 0,
    name: "Math Competition Student",
    designation: "AMC12 Qualifier",
    initials: "MC",
    rating: 5,
    content: (
      <p>
        The AES CHAMPIONS program helped me <Highlight>dramatically improve my competition scores</Highlight> through systematic training and problem-solving strategies. I qualified for AIME for the first time!
      </p>
    ),
  },
];
    ...
    const fetchTestimonials = async () => {
      try {
        ...
        if (data.success && data.testimonials && data.testimonials.length > 0) {
          // Filter testimonials for "AES CHAMPIONS Competitions"
          const championTestimonials = data.testimonials
            .filter((t: any) => {
              // Check if programs array includes "AES CHAMPIONS" or "Competition" (case-insensitive)
              return t.programs && Array.isArray(t.programs) && 
                     t.programs.some((p: string) => 
                       p.toLowerCase().includes('champion') || 
                       p.toLowerCase().includes('competition')
                     )
            })
            .map((t: any) => ({
              id: t.id || Math.random(),
          ...
          if (championTestimonials.length > 0) {
            setTestimonialCards(championTestimonials)
            setSelectedTestimonial(championTestimonials[0])
          }
        }
      } catch (error) {
    ...
    return (
      <div className="space-y-6">
        ...
        {!testimonial.successStory && !testimonial.content && (
          <p className="text-slate-200 text-base leading-relaxed">
            Great competition training experience!
          </p>
        )}
      </div>
    )
