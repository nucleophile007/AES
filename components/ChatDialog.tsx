...
import { Send, User, Wifi, WifiOff, RefreshCw } from "lucide-react";
import { useSocket, SocketMessage } from "../hooks/use-socket";
import MessageDebugger from "../app/components/ui/MessageDebugger";
...
export default function ChatDialog({
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      // Only show loading state for students, not for teachers
      if (userRole !== 'teacher') {
        setIsLoading(true);
      }
      
      // First try debug endpoint to diagnose issues
      const debugEndpoint = `/api/debug/messages?${userRole === 'teacher' ? 'teacherId' : 'studentId'}=${userId}&${userRole === 'teacher' ? 'studentId' : 'teacherId'}=${recipientId}&debug=true`;
      console.log('Fetching debug info from:', debugEndpoint);
      const debugResponse = await fetch(debugEndpoint);
      const debugData = await debugResponse.json();
      console.log('Debug data:', debugData);
      
      // Then fetch from regular endpoint
      ...
      if (response.ok && data.success && data.messages && data.messages.length > 0) {
        ...
        console.warn('Regular endpoint returned no messages or failed. Trying to use debug data.');
        
        // Try to use debug data if the regular endpoint failed
        if (debugData.success && debugData.filteredMessages && debugData.filteredMessages.length > 0) {
          ...
          const formattedMessages = debugData.filteredMessages.map((msg: any) => ({
            ...
            senderName: msg.senderId === userId ? userName : recipientName,
            senderRole: msg.senderRole
          }));
          
          console.log('Using messages from debug endpoint:', formattedMessages);
          setMessages(formattedMessages);
          setError(null);
          return;
        }
        
        console.error('Failed to fetch messages:', data.error || 'Unknown error', 'Status:', response.status);
        ...
        if (data.error === "Database operation failed") {
          ...
          // For database errors, suggest running the view creation script
          if (data.details?.includes('users_view')) {
            console.warn('The database view might be missing. Try running npm run db:view');
  ...
  const sendMessage = async () => {
    ...
    try {
      // Only show sending state for students, not for teachers
      if (userRole !== 'teacher') {
        setIsSending(true);
      }
  ...
  // Hide any spinners in the dialog content if this is a teacher chat
  const dialogClass = userRole === 'teacher' ? 'teacher-chat-no-spinner' : '';

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={`max-w-md sm:max-w-lg md:max-w-xl ${dialogClass}`}>
        <DialogHeader>
