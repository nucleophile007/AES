...
let content = fs.readFileSync('app/teacher-dashboard/page.tsx', 'utf8');

// 1. Add states for multiple files and links
...
// 3. Update the handleSendResource function
// I will replace the entire handleSendResource function
const handleSendResourceRegex = /const handleSendResource = async \(\) => \{[\s\S]*?showResourceSaveFeedback\([\s\S]*?\);\n    \}\n  \};/;

const newHandleSendResource = `const handleSendResource = async () => {
...
content = content.replace(handleSendResourceRegex, newHandleSendResource);

// 4. Update the "isResourceSubmitDisabled" and other booleans
content = content.replace(
...
content = content.replace(
  /const isResourceSubmitDisabled =[\s\S]*?;/,
  `const isResourceSubmitDisabled =
    sendingResource ||
    !newResource.title.trim() ||
    !newResource.type ||
    (newResourceFiles.length === 0 && newResourceLinks.length === 0 && currentLinkInput.trim() === "");`
);

// 5. Replace UI Card block for "Send Resource"
const uiCardRegex = /<Card className="border border-slate-200 bg-white shadow-sm transition-all hover:shadow-md">[\s\S]*?Send Resource to Students[\s\S]*?<\/CardTitle>[\s\S]*?<\/CardHeader>[\s\S]*?<\/CardContent>\s*<\/Card>/m;

const newUiCard = `<Card className="border border-slate-200 bg-white shadow-sm transition-all hover:shadow-md">
...
content = content.replace(uiCardRegex, newUiCard);

// 6. Filter resources logic in the "Shared Resources" render map
const renderMapRegex = /\{resources\.map\(\(resource\) => \{/g;
const newRenderMap = `{resources
  .filter((res) => {
  ...
  .map((resource) => {`;
content = content.replace(renderMapRegex, newRenderMap);

fs.writeFileSync('app/teacher-dashboard/page.tsx', content, 'utf8');
