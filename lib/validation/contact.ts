...
export const contactFormSchema = z.object({
  ...
  phone: z.preprocess(emptyToUndefined, z.string().min(7).max(25)).optional(),
  role: z.preprocess(emptyToUndefined, z.string().max(50)).optional(),
  programInterest: z.preprocess(emptyToUndefined, z.string().max(100)).optional(),
  subject: z.string().trim().min(3, "Subject is required").max(120),
  message: z.string().trim().min(10, "Message is required").max(2000),
  preferredContact: z.preprocess(emptyToUndefined, z.enum(["email", "phone", "either"]).optional()),
